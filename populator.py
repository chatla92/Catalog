...
for line in content:
       ...
       if any(ext not in details[0] for ext in ch):
           ...
           try:
               cat = session.query(Categories).filter_by(name=details[2].strip()).one()
           ...
           for ind in range(3):
               U = User(name=reviewers[ind], id=uid, email=reviewers[ind].replace(" ","")+"@amz.com")
               U.hash_password("1234")
               session.add(U)
               session.commit()
               R = Reviews(user_id=rid, product_id=details[1], review=reviews[ind])
               session.add(R)
               session.commit()
               print uid
               uid = uid+1
